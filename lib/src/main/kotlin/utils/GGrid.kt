...
inline fun <reified T> createGrid(width: Int, height: Int, valuesFn: (Vec2i) -> T): GGrid<T> {
  val arr = Array(width * height) { index -> valuesFn(Vec2i(index % width, index / width)) }
  return GGrid(arr, width, height)
}

/**
 * Generic equivalent of [Grid], the latter is Int-specialized for performance
 */
open class GGrid<T>(
  ...
  open class Column<T>(private val grid: GGrid<T>, private val x: Int) {
  ...
  open class Row<T>(private val grid: GGrid<T>, private val y: Int) {
  ...
  companion object {
    inline fun <reified T> empty() = GGrid<T>(emptyArray(), 0, 0)
...
val GGrid<Char>.debugString: String get() {
  return rows.map { it.values.joinToString("") }.joinToString("\n")
}

inline fun <reified T> GGrid<T>.borderWith(value: T, borderWidth: Int = 1): GGrid<T> {
...
inline fun <T, reified R> GGrid<T>.map(fn: (Vec2i, T) -> R) = createGrid(width, height) { pos -> fn(pos, this[pos.x][pos.y]) }
