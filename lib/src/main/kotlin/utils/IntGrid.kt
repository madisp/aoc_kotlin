...
open class IntGrid(
  ...
  open class Column(private val grid: IntGrid, private val x: Int) {
  ...
  open class Row(private val grid: IntGrid, private val y: Int) {
  ...
  fun borderWith(value: Int, borderWidth: Int = 1): IntGrid {
    return IntGrid(width + borderWidth * 2, height + borderWidth * 2) { (x, y) ->
  ...
  open fun map(fn: (Vec2i, Int) -> Int) = IntGrid(
    IntArray(arr.size) { i -> fn(Vec2i(i % width, i / width), arr[i]) },
    width,
    height
  )

  fun toMutable() = MutableIntGrid(arr.clone(), width, height)
  ...
  companion object {
    val EMPTY = IntGrid(intArrayOf(), 0, 0)
    ...
    private fun fromLines(input: List<String>, stride: Int, delimiter: String = " "): IntGrid {
      val nums = input.filter(String::isNotBlank).flatMap {
        it.split(delimiter).filter(String::isNotBlank).map(String::toInt)
      }

      return IntGrid(nums.toIntArray(), stride, nums.size / stride)
    }

    private fun fromDigits(input: String, stride: Int): IntGrid {
